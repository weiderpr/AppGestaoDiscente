...
class TestNoBonusScheduler extends \App\Services\SomativaScheduler {
    public function runTest($somativaId, $instId) {
        ...
        $success = $this->solveBacktrackTest(
    ...
    private function solveBacktrackTest(
        int $queueIdx,
        array $queue,
        array $data,
        array $restricoes,
        ...
        $calls++;
        if ($calls > 300000) return false;
        if (count($solutions) >= 10000) return true;

        if ($queueIdx >= count($queue)) {
            $solutions[] = $alocacoes;
            return false; // Force backtrack to search all
        }
        ...
        if (isset($preAllocated[$sdId])) {
            return $this->solveBacktrackTest(
        ...
        foreach ($datesNormais as $date) {
            ...
            foreach ($data['slots'] as $slot) {
                ...
                // REMOVE THE DAY INDEX BONUS from score!
                $dayIdx = array_search($date, $data['dates']);
                if ($dayIdx !== false) {
                    $dayBonus = max(0, 8 - (int)($dayIdx * 0.4));
                    $score -= $dayBonus;
                }
